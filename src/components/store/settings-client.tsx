...
import { createVoucherAction, toggleVoucherAction, deleteVoucherAction, updateDomainAction } from "@/app/store/settings/actions";
...
export function SettingsClient({
    ...
    const handleSaveDomain = async () => {
        await updateDomainAction(domain);
        toast.success("Domain updated successfully");
    };

    const handleSaveStoreCustomization = async () => {
        // We are reusing updateDomainAction for settings, rename could be ideal but this mocks it out.
        // In a real app we'd dispatch an updateSettingsAction
        const { updateStoreSettingsAction } = await import("@/app/store/settings/actions");
    ...
    const handleToggleVoucher = async (id: string, active: boolean) => {
        ...
        await toggleVoucherAction(id, !active);
                    ...
                    <CardContent className="flex flex-col items-center">
                        {qrCodeUrl && (
                            <div className="bg-white p-4 rounded-xl shadow-sm mb-4">
                                <Image src={qrCodeUrl} alt="Store QR Code" width={200} height={200} />
                            </div>
                        )}
